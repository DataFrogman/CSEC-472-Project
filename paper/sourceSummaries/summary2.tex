...
\noindent
The authors directly compare different authentication methods using different factors.  These can include battery life, cryptographic and mechanical security, the concern of a passed credential, and what happens if an authentication token is lost.  

\subsubsection{Findings}

\noindent
The authors found that each system has individual strengths.  As each method does better in different ways, emphasis is placed on ensuring the use of the lock that is best suited for the use case in which it is placed.  Factors considered include but are not limited to whether a physical key is required, whether or not the key will be passed off to a third party user, and if the system would be considered secure for the given use case.
